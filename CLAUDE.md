...
`tests/validate_recipes.py` is where the constraints below are enforced. It checks what the schema cannot see — a recipe-local `extensions:` block, a missing `prompt`, an `${VAR}` absent from `env_keys`, a write-capable recipe without the `-apply` suffix, a read-only recipe whose policy block names only a subset of the state-changing tools, a named skill with no directory, an agent in the directory this provider cannot see — and runs without a provider call. Adding a house rule means adding a check there, not only a paragraph to the spec.
...
- A recipe that changes backend state carries an `-apply` filename suffix, opens its `description` with `WRITES to …`, and states the effect there — Kamerplanter's `mcp.write` / `mcp.setup` tools, and any Home Assistant tool that actuates. Only the first half is machine-checkable; HA's catalog is per-instance. The suffix is about a garden, not the filesystem — `domain-review` writes a report into the gitignored `.audits/` tree, says so in its `description`, and deliberately does not take the suffix.
