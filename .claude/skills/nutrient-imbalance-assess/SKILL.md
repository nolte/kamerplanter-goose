...
- `mcp__kamerplanter__record_feeding_event` records a fertigation with its amount and the EC and pH before and after, but it writes, so a read-only run must not call it — the fertigation is recorded outside the run. Be precise about what that buys: **no tool measured so far reads those events back**, and the evidence this skill judges is the feeding frequency from `mcp__kamerplanter__get_plant_care_log` plus whatever `measurements` the caller's diary entries carry. Logging improves the garden's record; on its own it does not lift this run's tier. Do not invent a read call for it — but do not assume its absence either: the catalog is a floor rather than a contract, so re-measure before stating that no such tool exists. For growers who confirm reminders but log no quantities, tier 2 collapses to tier 3, and the honest output is a refusal plus a request for one EC or pH reading in a diary entry — the tier 1 value that does settle the direction.

## Source

Derived from `spec/process/nutrient-imbalance-detection/` §"The evidence ladder" and §"Deciding the direction", measured against the reference instance on 2026-08-05. The tool names and the write/read split come from `spec/mcp/kamerplanter-mcp-server/` instead, whose catalog was last re-measured on 2026-08-16; `spec/process/` predates `record_feeding_event` and names it nowhere.
