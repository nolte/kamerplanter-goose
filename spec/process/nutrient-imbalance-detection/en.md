...
- **MUST** read the plant's recent diary entries before assessing — listing them for the plant, then reading each — since tier 1 lives in `measurements` and no other step fetches it. A process that skips this can never rise above tier 3 and refuses every time, including for a plant whose diary holds the reading. It **MUST NOT** filter by entry type: `measurements` is carried by any entry regardless of type, and the enum holds six, so a filter hides the reading this step exists to find. Filter and paging parameters are deliberately unnamed here — `list_diary_entries` has no measured signature, and a rejected argument ends the step in a `validation.error`, dropping the run back to the refusal this requirement removes
...
- `list_diary_entries` was **not** among the tools measured that day. Its filter, limit, ordering and paging parameters are therefore unverified, which is why no requirement above names one.
