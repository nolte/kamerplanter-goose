...
- **MUSS [MUST]** vor der Beurteilung die jüngsten Tagebucheinträge der Pflanze lesen — sie für die Pflanze auflisten, dann jeden einzeln lesen —, denn Stufe 1 liegt in `measurements` und kein anderer Schritt holt sie. Ein Verfahren, das das überspringt, kommt nie über Stufe 3 hinaus und verweigert jedes Mal, auch bei einer Pflanze, deren Tagebuch den Wert enthält. Es **DARF NICHT [MUST NOT]** nach Eintragstyp filtern: `measurements` wird von jedem Eintrag unabhängig vom Typ getragen, und das Enum kennt sechs — ein Filter verbirgt genau den Wert, für den dieser Schritt existiert. Filter- und Paging-Parameter bleiben hier bewusst unbenannt: `list_diary_entries` hat keine gemessene Signatur, und ein abgewiesenes Argument beendet den Schritt mit einem `validation.error` und wirft den Lauf auf die Verweigerung zurück, die diese Anforderung beseitigt
