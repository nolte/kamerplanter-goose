...
- **MUST** carry an `-apply` filename suffix and state the effect in `description` when a recipe calls a tool that changes a backend's state: Kamerplanter's `mcp.write` and `mcp.setup` classes, and any Home Assistant tool that actuates — turning a device on or off, setting a value, running a script, scene, or automation. Only the Kamerplanter half is machine-checkable, because that catalog is enumerable and Home Assistant's is assembled per instance; the actuating half rests on [the Home Assistant server spec](../../mcp/home-assistant-mcp-server/en.md) and on review
- **MUST NOT** take the suffix for a recipe that changes no backend state and only writes a file into the gitignored `.audits/` tree, and **MUST** say what it writes in its `description` instead. The suffix is a statement about somebody's garden, not about the filesystem, and spending it on a report nobody can lose devalues it where it matters
...
- **MUST** declare its tool policy in the `prompt` under two literal markers — `Forbidden by name:` and, for an `-apply` recipe, `Permitted by name:` — each opening a bullet whose body is nothing but backticked tool names and punctuation. A name inside such a block is policy; a name outside it is a call. The same convention governs a skill's hard rules, and the two are read together: a recipe is orchestration and the calls live in the procedures it loads, so judging the YAML alone leaves a skill free to write with nothing noticing. An explanation goes on its own line — where names and prose share one, every word of that prose is a place a call sits unseen, because the block is cut out before the text is read. Deciding that by convention rather than by reading the surrounding English is what makes it checkable: seven review rounds went into a parser that inferred it from sentence and list structure, and it misread both directions
- **MUST**, where it carries the `-apply` suffix, call only the state-changing tools its own `Permitted by name:` block lists. The suffix announces that a recipe writes; it does not say which writes, and a reader who granted the run on the strength of its `description` has no other place to learn the answer
- **MUST** forbid, in its `prompt`, every server the shared configuration loads that the recipe does not use. Measured: `extensions.yaml` loads three, `.envrc` supplies a live credential for each, and no recipe here calls a GitHub tool — so without a prohibition every plant-care run holds a repository write surface it has no use for. A blanket is required rather than an enumeration where the catalog is assembled per instance or can only be split into reads and writes by name
- **MUST NOT** dispatch a plugin-provided agent type from a recipe; only the built-in types and those defined in the working directory's `.claude/agents/` are in the registry a run sees. The single exception is a probe measuring that very fact: `provider-plugin-check` attempts one on purpose and reports the error verbatim, which is where the "fails" row in the table above comes from
- **SHOULD** ship a connectivity recipe that exercises read-only every declared server the recipes actually use, and reports per-server PASS/FAIL, and **SHOULD** run it before trusting any other recipe against a new environment. A loaded but unused server is deliberately left unprobed: the MUST above forbids it in every prompt, and probing it would be the one call the repository makes to a surface it has no business with. The cost is accepted and named — an extension whose `env_keys` variable is unset disappears with no warning, and for such a server nothing here would surface that
...
- [ ] No recipe dispatches a plugin-provided agent type, except a probe whose reported result is the failure itself
- [ ] Every project agent lives under `.claude/agents/`, not `.agents/agents/`
- [ ] Every recipe forbids each loaded server it does not use, in its `prompt`
- [ ] Every state-changing tool a recipe names sits under `Forbidden by name:` or `Permitted by name:`
- [ ] No `-apply` recipe calls a state-changing tool its `Permitted by name:` block omits
- [ ] No policy block line carries both a tool name and prose, in a recipe or in a skill
- [ ] Every recipe that dispatches a project agent names the required working directory in its `description`
