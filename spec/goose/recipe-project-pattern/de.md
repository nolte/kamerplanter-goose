...
- **MUSS [MUST]** seine Werkzeugpolitik im `prompt` unter zwei wörtlichen Markern führen — `Forbidden by name:` und, bei einem `-apply`-Rezept, `Permitted by name:` —, die jeweils einen Block eröffnen, dessen Rumpf aus in Backticks gesetzten Werkzeugnamen und Satzzeichen besteht. Ein Name innerhalb eines solchen Blocks ist Politik, ein Name außerhalb aller Blöcke ein Aufruf. Das über eine Konvention statt über das Lesen der umgebenden Prosa zu entscheiden, macht es überhaupt prüfbar: Zwölf Review-Runden flossen in einen Parser, der es aus Satz- und Listenstruktur erschloss, und er las es durchgehend in beide Richtungen falsch. Ein Projekt-Skill, der eine eigene Werkzeugpolitik führt, verwendet dieselben zwei Marker statt einer zweiten Konvention — die Prüfung, die sie liest, ist dieselbe, und ein Skill, dessen Verbot in gewöhnlicher Prosa steht, deklariert nichts maschinell Prüfbares
...
- [ ] Jedes Rezept, und jeder Skill, der überhaupt ein zustandsänderndes Tool nennt, nennt alle zwölf *innerhalb* eines `Forbidden by name:`- oder `Permitted by name:`-Blocks — ein Name in der umgebenden Prosa zählt nicht
