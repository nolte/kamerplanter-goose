...
- **MUSS [MUST]** seine Werkzeugpolitik im `prompt` unter zwei wörtlichen Markern führen — `Forbidden by name:` und, bei einem `-apply`-Rezept, `Permitted by name:` —, die jeweils einen Aufzählungspunkt eröffnen, dessen Rumpf nur aus in Backticks gesetzten Werkzeugnamen und Satzzeichen besteht. Ein Name innerhalb eines solchen Blocks ist Politik, ein Name außerhalb ein Aufruf. Dieselbe Konvention gilt für die harten Regeln eines Skills, und beide werden zusammen gelesen: Ein Rezept ist Orchestrierung, die Aufrufe stehen in den Prozeduren, die es lädt — wer nur das YAML prüft, lässt einem Skill das Schreiben frei. Eine Erläuterung steht auf einer eigenen Zeile: Wo Namen und Prosa sich eine Zeile teilen, ist jedes Wort dieser Prosa eine Stelle, an der ein Aufruf ungesehen sitzt, denn der Block wird herausgeschnitten, bevor der Text gelesen wird. Das über eine Konvention statt über das Lesen der umgebenden Prosa zu entscheiden, macht es überhaupt prüfbar: Sieben Review-Runden flossen in einen Parser, der es aus Satz- und Listenstruktur erschloss, und er las es in beide Richtungen falsch
- **MUSS [MUST]**, sofern es den Suffix `-apply` trägt, nur die zustandsändernden Werkzeuge aufrufen, die sein eigener `Permitted by name:`-Block auflistet. Der Suffix kündigt an, dass ein Rezept schreibt; er sagt nicht, was — und wer den Lauf auf Basis der `description` freigegeben hat, hat keine andere Stelle, an der er es erfährt
- **MUSS [MUST]** in seinem `prompt` jeden von der geteilten Konfiguration geladenen Server verbieten, den es nicht nutzt. Gemessen: `extensions.yaml` lädt drei, `.envrc` liefert für jeden ein gültiges Zugangsdatum, und kein Rezept hier ruft ein GitHub-Tool auf — ohne Verbot hält also jeder Pflanzenpflege-Lauf eine Repository-Schreiboberfläche, für die er keine Verwendung hat. Wo ein Katalog je Instanz zusammengesetzt wird oder sich nur über Namen in Lesen und Schreiben teilen lässt, ist ein Pauschalverbot statt einer Aufzählung verlangt
- **DARF NICHT [MUST NOT]** einen von einem Plugin bereitgestellten Agent-Typ aus einem Rezept dispatchen; in der Registry, die ein Lauf sieht, stehen nur die eingebauten Typen und die aus `.claude/agents/` des Arbeitsverzeichnisses. Die einzige Ausnahme ist eine Sonde, die genau das misst: `provider-plugin-check` versucht es absichtlich und zitiert den Fehler wörtlich — daher stammt die Zeile „scheitert" in der Tabelle oben
- **SOLLTE [SHOULD]** ein Konnektivitätsrezept ausliefern, das jeden deklarierten Server, den die Rezepte tatsächlich nutzen, nur lesend prüft und je Server PASS/FAIL meldet, und **SOLLTE [SHOULD]** es ausführen, bevor einem anderen Rezept in einer neuen Umgebung vertraut wird. Ein geladener, aber ungenutzter Server bleibt bewusst ungeprüft: Das MUSS oben verbietet ihn in jedem Prompt, und ihn zu prüfen wäre der eine Aufruf, den das Repository auf eine Oberfläche macht, mit der es nichts zu tun hat. Der Preis ist akzeptiert und benannt — eine Extension, deren `env_keys`-Variable nicht gesetzt ist, verschwindet ohne jede Warnung, und für einen solchen Server würde das hier nichts sichtbar machen
...
- [ ] Kein Rezept dispatcht einen von einem Plugin bereitgestellten Agent-Typ, außer einer Sonde, deren gemeldetes Ergebnis genau dieser Fehlschlag ist
- [ ] Jeder Projekt-Agent liegt unter `.claude/agents/`, nicht unter `.agents/agents/`
- [ ] Jedes Rezept verbietet in seinem `prompt` jeden geladenen Server, den es nicht nutzt
- [ ] Jedes zustandsändernde Tool, das ein Rezept nennt, steht unter `Forbidden by name:` oder `Permitted by name:`
- [ ] Kein `-apply`-Rezept ruft ein zustandsänderndes Werkzeug auf, das sein `Permitted by name:`-Block auslässt
- [ ] Keine Policy-Block-Zeile trägt Werkzeugnamen und Prosa zugleich, weder im Rezept noch im Skill
- [ ] Jedes Rezept, das einen Projekt-Agent dispatcht, benennt das erforderliche Arbeitsverzeichnis in seiner `description`
