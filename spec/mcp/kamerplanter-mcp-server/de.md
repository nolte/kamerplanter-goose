...
Zwei Eigenschaften machen eine Spezifikation lohnender als eine Entdeckung pro Rezept. Erstens ist der Werkzeugkatalog **aufzählbar und vollständig schematisiert** — jedes Tool deklariert ein komplettes JSON Schema, sodass die gesamte Oberfläche durch eine einzige Abfrage vorab bekannt ist. Das ist der scharfe Gegensatz zu [Home Assistant](../home-assistant-mcp-server/de.md), dessen Katalog zur Laufzeit zusammengesetzt wird und pro Instanz abweicht. Aufzählbar heißt aber nicht fest: Zwischen zwei Messungen im Abstand von drei Tagen wuchs der Katalog von 12 auf 43 Tools und neun Tage später auf 56 — die Oberfläche ist also je Instanz und je Datum erkennbar, nicht ein für alle Mal. Zweitens ist der Server **mandantenfähig mit einem Berechtigungsmodell pro Garten**, sodass derselbe Schlüssel in einem Garten schreiben darf und in einem anderen dieselbe Aktion verweigert bekommt. Ein Rezept, das dies ignoriert, erzeugt Fehler, die wie Bugs aussehen, aber korrekte Ablehnungen sind.

Alles unten als *gemessen* Gekennzeichnete wurde von der Referenzinstanz gelesen — Transport, Authentifizierung und Mandantenfähigkeit am 2026-08-04 über `initialize`, `tools/list` und nur lesende `tools/call`-Aufrufe; der Werkzeugkatalog und seine Feldstrukturen am 2026-08-07 auf demselben Weg; der Katalog am 2026-08-16 neu gezählt anhand der Werkzeugoberfläche, die eine Goose-Sitzung erhält, wobei das `inputSchema` jedes neuen Tools von dieser Oberfläche gelesen und mehrere davon aufgerufen wurden. Die Upstream-Dokumentation kennzeichnete den Server bei der ersten Messung als **teilweise verfügbar**, mit 12 von rund 30 spezifizierten Tools; die zweite Messung fand 43, die dritte 56. Die Implementierung hat diesen Stand also überholt, und die Dokumentation ist für den Umfang nicht mehr maßgeblich.
...
Der Katalog wächst zwischen zwei Messungen. Eine Inventur am 2026-08-04 ergab 12 Tools und deckte sich mit der damaligen Dokumentation; eine erneute Inventur am 2026-08-07 ergab **43** — 31 mehr, davon 28 Lesetools und drei Schreibtools (`add_plant_diary_entry`, `claim_diary_analysis`, `submit_diary_analysis`). Eine dritte Zählung am 2026-08-16 ergab **56** — 13 mehr, davon acht Lesetools und fünf Schreibtools. Der Zuwachs ist eine zusammenhängende Schicht und keine verstreute Ergänzung: die Oberfläche für Lebenszyklus und Phasenabläufe (`get_plant_phase_status`, `get_plant_phase_history`, `get_species_lifecycle`, `get_species_phase_sequence`, `list_phase_sequences`, `list_species_by_phase_sequence`, `assign_species_phase_sequence`, `transition_plant_phase`), dazu Diagnostik, IPM-Rückschreibung, Düngeereignisse, Nährstoffplan-Zuweisung und eine Wissenssuche. **Jede Zahl hier ist eine Untergrenze, keine Zusicherung** — vor der Annahme, ein Tool fehle, ist `tools/list` erneut abzufragen.

Zwei gemessene Vorbehalte zur Oberfläche vom 2026-08-16, denn ein deklariertes Tool ist nicht dasselbe wie ein funktionierendes:

- **`search_plant_knowledge` ist deklariert, hier aber nicht ausgerollt.** Es antwortet `service.unavailable: The knowledge base is not reachable right now; retry later. It is an optional component and may not be deployed on this instance.` — eine saubere, typisierte Ablehnung und genau die Form, die ein Rezept von einer optionalen Komponente erwarten sollte.
- **`get_mcp_activity` antwortete bei jedem Aufruf mit `INTERNAL_ERROR`** (drei Versuche, verschiedene `limit`-Werte, unterschiedliche Referenz-IDs). Das ist ein Serverdefekt und keine Katalogeigenschaft — gemeldet als `nolte/kamerplanter#1145` —, aber ein Rezept, das einen Selbstaudit-Schritt plant, darf nicht voraussetzen, dass das Tool antwortet.

### Lesetools (`mcp.read`), 44 gemessen
...
| Pflanzen | `list_plants`, `get_plant`, `list_plants_at_location`, `list_planting_runs`, `get_plant_diagnostics` |
| Arten und Sorten | `list_species`, `get_species_info`, `list_cultivars`, `get_cultivar`, `get_species_lifecycle` |
| Phasen und Lebenszyklus | `list_phase_definitions`, `get_sowing_calendar`, `list_overwintering_profiles`, `list_hardiness_zones`, `get_plant_phase_status`, `get_plant_phase_history`, `get_species_phase_sequence`, `list_phase_sequences`, `list_species_by_phase_sequence` |
...
| Einstieg und Fachvokabular | `list_starter_kits`, `search_glossary`, `search_plant_knowledge` |
...
| `get_plant_phase_status` | `plant_key` | ja | Löst die Phase der Pflanze auf; antwortet `phase_state: unresolved`, wenn der offene Satz auf keine Phase zeigt — das liest sich wie ein normales Ergebnis und ist keines |
| `get_species_phase_sequence` | `species_key` | nein | Liefert `sequence: null` für eine ungebundene Art — ein gewöhnlicher Zustand, kein Fehler |
| `list_phase_sequences` | — | nein | 22 Abläufe gemessen; `include_entries` standardmäßig aus |
| `search_plant_knowledge` | `query` | nein | Optionale Komponente; siehe den Vorbehalt oben |

`calculate_mixing_protocol` rechnet und persistiert nichts — deshalb steht es trotz seines Namens bei den Lesetools.

Zwanzig Lesetools nehmen kein `tenant`: `list_tenants` und `get_mcp_activity` sind kontobezogen, und die Kataloge für Arten, Sorten, Schädlinge, Krankheiten, Behandlungen, Substrate, Glossar, Winterhärtezonen, Phasendefinitionen, Phasenabläufe, Art-Lebenszyklen und Wissensbasis gelten gartenübergreifend. Die Aufteilung gilt je Tool und wurde aus dem jeweiligen `inputSchema` gelesen, nicht aus der Gruppe erschlossen, in der ein Tool steht: `list_substrates` nimmt kein `tenant`, `list_fertilizers` in derselben Gruppe schon, und `get_species_phase_sequence` nimmt keines, während `get_plant_phase_status` in derselben Gruppe eines nimmt. Für die übrigen 24 gilt die Mandantenregel von oben — optional bei einem Schlüssel mit einer Mitgliedschaft, verpflichtend bei mehreren —, aus diesem Abschnitt lässt sich also nicht ableiten, dass das Weglassen je unbedenklich wäre.
...
| `transition_plant_phase` | `plant_key`, `target_phase_key` | Eine Pflanze in eine Phase ihres aufgelösten Ablaufs überführen oder eine falsche reparieren |
| `assign_species_phase_sequence` | `species_key`, `sequence_key` | Eine Art an einen bestehenden Ablauf binden — **nimmt kein `tenant`**, siehe unten |
| `assign_nutrient_plan` | `plant_key`, `plan_key` | Einen bestehenden Nährstoffplan an eine Pflanze binden |
| `record_feeding_event` | `plant_key`, `volume_applied_liters` | Eine Fertigation festhalten: Menge, EC/pH davor und danach, Ablauf |
| `create_inspection` | `plant_key` | Eine IPM-Inspektion festhalten: Druckstufe, Symptome, strukturierte Befunde |
...
Gemessen: Jedes zustandsändernde Tool bietet `dry_run` und `idempotency_key` an, und kein Lesetool trägt eines davon — das ist der billigste Weg, die beiden Klassen in einer `tools/list`-Antwort zu unterscheiden. Das galt am 2026-08-16 für alle zwölf und ist damit das einzige Unterscheidungsmerkmal, das jedes Katalogwachstum überstanden hat.

`tenant` trennt sie **nicht**, und seit dem 2026-08-16 gilt es nicht einmal mehr innerhalb der Schreibklasse. Elf der zwölf zustandsändernden Tools nehmen `tenant`, ebenso 24 der 44 Lesetools. Die Ausnahme ist **`assign_species_phase_sequence`, das keines nimmt**: Beide Argumente benennen Zeilen in gartenübergreifenden Katalogen — eine Art und einen Phasenablauf —, der Aufruf ist also überhaupt nicht auf einen Garten bezogen. Seine Wirkung ist es ebenso wenig: Eine Art neu zu binden ändert den aufgelösten Ablauf dieser Art in *jedem* Mandanten. Ein Rezept, das „kein `tenant`-Argument" als „harmlos" liest, hat die Schlussrichtung hier verkehrt.

Für die übrigen elf sind Schreibtools wie alles andere mandantengebunden, die Mandantenregel von oben gilt für sie also vollständig: Bei einem Schlüssel über mehrere Gärten ist ein Schreibaufruf ohne `tenant` ein Fehler, kein Standardwert.
...
- **MUSS [MUST]** jedes verbotene zustandsändernde Tool im `prompt` des Rezepts einzeln benennen, wenn das Rezept nur liest — `instructions` allein wird in einem Headless-Lauf nicht durchgesetzt. Aufeinanderfolgende Katalogmessungen haben diese Liste von vier auf sieben und dann auf zwölf gehoben: `confirm_care_task`, `archive_plant`, `set_plant_location`, `create_site`, `add_plant_diary_entry`, `claim_diary_analysis`, `submit_diary_analysis`, `transition_plant_phase`, `assign_species_phase_sequence`, `assign_nutrient_plan`, `record_feeding_event`, `create_inspection` (elf `mcp.write` plus das eine `mcp.setup`). Eine Teilmenge zu nennen ist der Fehler, für den diese Regel existiert, und eine Kategorie („und alles andere, was schreibt") ersetzt keinen der zwölf Namen: Das Verbot darf nicht von der Klassifikation des Modells abhängen. Die Liste ist zweimal gewachsen; ein gegen einen früheren Stand geschriebenes Rezept ist heute eine Teilmenge — deshalb trägt die maschinelle Prüfung weiter unten die Regel und nicht dieser Absatz. Ein schreibfähiges `-apply`-Rezept ist die Ausnahme, die das Wort *verboten* bereits enthält — die Werkzeuge, für die es existiert, werden deklariert, nicht verboten
...
- Das Kriterium zu den verbotenen Tools wurde nun dreimal durch denselben Mechanismus verfehlt, und das dritte Mal geschah innerhalb der Änderung, die das zweite behob. Die Messung vom 2026-08-07 hob die Liste auf sieben, und die Rezepte wurden darauf gebracht. Die Messung vom 2026-08-16 hob sie auf zwölf, und **neun der zehn ausgelieferten Rezepte nannten gemessen sieben** — jedes gegen den älteren Stand geschriebene war still zu einer Teilmenge geworden. Der erste Korrekturlauf in derselben Änderung behob nur drei, weil er die Rezeptliste aus einer früheren Fassung *dieses Absatzes* übernahm, statt die Rezepte zu messen; ein Review fand die übrigen sechs. Alle zehn nennen jetzt zwölf.
  ...
  Zweitens die Lücke, die haltbar war, bis sie geschlossen wurde: `tests/validate_recipes.py` setzte lange die *`-apply`-Suffix*-Regel durch (ein Rezept, das ein Schreibtool außerhalb eines Policy-Blocks nennt, muss den Suffix tragen), nicht aber die *Vollständigkeits*-Regel (ein nur lesendes Rezept muss jedes zustandsändernde Tool nennen). Nur eine Validator-Prüfung für die zweite Regel hätte einen der drei Rückfälle gefangen. Es gibt sie jetzt; sie bezieht ihre Liste aus `STATE_CHANGING_TOOLS` — derselben Konstante, die die Katalogmessung pflegt —, gilt für jedes Rezept mit einem `prompt` und misst allein an diesem `prompt`, weil `instructions` in einem Headless-Lauf nicht durchgesetzt wird; ein Rezept ganz ohne `prompt` wird bereits von seiner eigenen Prüfung abgewiesen, und eine zusätzliche Meldung dazu würde diese Ursache verschütten. Sie auf Rezepte zu beschränken, die diesen Server nachweislich erreichen, wurde versucht und zurückgenommen: Die bloße Namensform ist ohne eine Liste der Lese-Tools nicht erkennbar, und genau eine solche Liste ist der Veralterungsdefekt, den diese Prüfung fangen soll.

  Zweierlei leistet sie bewusst nicht. Sie begrenzt nicht, was ein `-apply`-Rezept unter `Permitted by name:` deklariert — eines, das dort alle zwölf führt, besteht; dafür bräuchte es ein Register erlaubter Writes je Rezept. Und eine Verbotsliste, die als ein kommentiertes Bullet je Tool oder als Tabelle geschrieben ist, endet am Marker und wird abgewiesen — ebenso eine Konjunktion vor dem letzten Namen: `` `a`, and `b`. `` lässt `b` außerhalb des Blocks stehen und als Aufruf melden. Die Namen müssen zusammenstehen, ununterbrochen von Prosa, die Erläuterung gehört hinter den Block. Ein Toolname in einem umzäunten Beispiel zählt dagegen als deklariert: Das ist der eine Weg, ein Rezept vollständiger aussehen zu lassen, als es ist, und bewusst als die harmlose Richtung offengelassen.
...
- Diese Spec hat keinen Mechanismus, um zu erkennen, dass der laufende Server Tools anbietet, die sie nicht führt. Zum Stand der Nachinventur vom 2026-08-16 stimmen beide überein — alle 56 sind hier aufgeführt —, aber nichts hält sie in Übereinstimmung. Die Drift fiel inzwischen zweimal von Hand auf: drei Tage nachdem sie in der ersten Augustwoche entstand, und neun Tage nachdem sie ein zweites Mal entstand, beide Male beiläufig und nicht durch eine Prüfung. Davon unabhängig: Upstream dokumentiert den Katalog weiterhin als auf rund 30 wachsend, was die Implementierung inzwischen doppelt überholt hat.
...
- Zuordnung von `tenant`, `dry_run` und `idempotency_key` je Tool, abgeleitet durch Prüfung von `inputSchema.properties` auf diese Schlüssel — nicht aus der Gruppierung in diesem Dokument. Für die 43 Tools vom 2026-08-07 stammt sie aus der `tools/list`-Antwort ohne jeden Aufruf, diese Aussagen betreffen also das deklarierte Schema und nicht beobachtetes Verhalten; für die bis 2026-08-16 hinzugekommenen 13 wurden die Schemata von der Oberfläche gelesen, die eine Goose-Sitzung erhält — also aus derselben Antwort, wie der Client sie darbietet
...
- Live-Messung der Referenzinstanz, 2026-08-16, im Rahmen einer Pflanzenkonfigurations-Prüfung statt einer eigenen Sonde: 56 Tools auf der Sitzungsoberfläche, nur lesende `tools/call` für `list_tenants`, `list_plants`, `get_plant`, `list_plants_at_location`, `get_plant_diagnostics`, `get_plant_phase_status`, `get_plant_phase_history`, `get_species_info`, `get_species_lifecycle`, `get_species_phase_sequence`, `list_phase_sequences`, `list_species`, `list_cultivars`, `list_substrates`, `list_fertilizers`, `list_nutrient_plans`, `get_nutrient_plan`, `get_plant_nutrient_plan`, `calculate_mixing_protocol`, `get_due_care_tasks`, `list_tasks`, `get_harvest_readiness`, `list_planting_runs`, dazu `get_mcp_activity` und `search_plant_knowledge`, die beide wie oben vermerkt fehlschlugen. **Diesmal wurden Schreibtools aufgerufen**, anders als bei den früheren Messungen: `assign_species_phase_sequence` und `transition_plant_phase` waren erfolgreich, `assign_nutrient_plan` scheiterte auf dem Commit-Pfad mit `INTERNAL_ERROR`, während sein `dry_run` durchlief (`nolte/kamerplanter#1145`). Damit ist `dry_run` bei mindestens einem Schreibtool als echter No-op bestätigt — und zugleich als **kein** Beleg dafür, dass der Schreibvorgang selbst gelingt
