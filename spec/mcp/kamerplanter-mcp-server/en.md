...
- The forbidden-tool criterion has now been missed twice by the same mechanism. The 2026-08-07 measurement raised the list to seven, and three recipes were brought up to it. The 2026-08-16 measurement raised it to twelve, and those same three — `connectivity-check`, `provider-surface-check`, `provider-plugin-check` — were measured naming seven, having silently become subsets again. They were corrected in the same change as this re-measurement. The recurring gap is that `tests/validate_recipes.py` enforces the *`-apply` suffix* rule (a recipe naming a write tool outside a policy block must carry the suffix) but not the *completeness* rule (a read-only recipe must name every one). A validator check for the second rule is the only thing that would have caught either regression; without it the criterion stays a statement of intent that holds only until the catalog grows again.
