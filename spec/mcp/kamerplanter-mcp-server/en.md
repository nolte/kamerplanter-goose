...
Two properties make it worth specifying rather than discovering per recipe. First, the tool catalog is **enumerable and fully schema'd** — every tool declares a complete JSON Schema, so a recipe author can know the whole surface ahead of time by asking once. This is the sharp contrast to [Home Assistant](../home-assistant-mcp-server/en.md), whose catalog is assembled at runtime and differs per instance. Enumerable is not the same as fixed: the catalog grew from 12 tools to 43 between two measurements three days apart, and to 56 nine days after that, so the surface is knowable per instance and per date rather than once and for all. Second, the server is **multi-tenant with a per-garden permission model**, so the same key may write in one garden and be refused the identical action in another. A recipe that ignores this produces errors that look like bugs but are correct refusals.

Everything below marked *measured* was read from the reference instance — the transport, authentication and tenancy sections on 2026-08-04 via `initialize`, `tools/list` and read-only `tools/call`; the tool catalog and its field shapes on 2026-08-07 the same way; the catalog re-counted on 2026-08-16 from the tool surface a Goose session is given, with each new tool's `inputSchema` read from that surface and several of them called. The upstream documentation marked the server **partially available** at the first measurement, with 12 of roughly 30 specified tools implemented; the second found 43 and the third 56, so the implementation has overtaken that reading and the documentation is no longer the authority on scope.
...
## Tool Catalog (measured, 56 tools, 2026-08-16)
...
The catalog grows between measurements. An inventory on 2026-08-04 returned 12 tools and matched the documentation of the day; a re-inventory on 2026-08-07 returned **43** — 31 more, of which 28 are read tools and three are writes (`add_plant_diary_entry`, `claim_diary_analysis`, `submit_diary_analysis`). A third count on 2026-08-16 returned **56** — 13 more, of which eight are reads and five are writes, and the growth is one coherent layer rather than scattered additions: the lifecycle and phase-sequence surface (`get_plant_phase_status`, `get_plant_phase_history`, `get_species_lifecycle`, `get_species_phase_sequence`, `list_phase_sequences`, `list_species_by_phase_sequence`, `assign_species_phase_sequence`, `transition_plant_phase`), plus diagnostics, IPM write-back, feeding events, nutrient-plan assignment and a knowledge search. **Treat any count here as a floor, not a contract**, and re-run `tools/list` before assuming a tool is absent.

Two measured caveats on the 2026-08-16 surface, because a declared tool is not the same as a working one:

- **`search_plant_knowledge` is declared but not deployed here.** It answers `service.unavailable: The knowledge base is not reachable right now; retry later. It is an optional component and may not be deployed on this instance.` — a clean, typed refusal, and the shape a recipe should expect from an optional component.
- **`get_mcp_activity` answered `INTERNAL_ERROR` on every call** (three attempts, differing `limit`, distinct reference IDs). That is a server defect rather than a catalog property — filed as `nolte/kamerplanter#1145` — but a recipe planning a self-audit step must not assume the tool answers.

### Read tools (`mcp.read`), 44 measured
...
| Plants | `list_plants`, `get_plant`, `list_plants_at_location`, `list_planting_runs`, `get_plant_diagnostics` |
| Species and varieties | `list_species`, `get_species_info`, `list_cultivars`, `get_cultivar`, `get_species_lifecycle` |
| Phases and lifecycle | `list_phase_definitions`, `get_sowing_calendar`, `list_overwintering_profiles`, `list_hardiness_zones`, `get_plant_phase_status`, `get_plant_phase_history`, `get_species_phase_sequence`, `list_phase_sequences`, `list_species_by_phase_sequence` |
...
| Onboarding and vocabulary | `list_starter_kits`, `search_glossary`, `search_plant_knowledge` |
...
| `get_plant_phase_status` | `plant_key` | yes | Resolves the plant's phase; answers `phase_state: unresolved` when the open record points at no phase, which reads as a normal result and is not |
| `get_species_phase_sequence` | `species_key` | no | Returns `sequence: null` for an unbound species — an ordinary state, not an error |
| `list_phase_sequences` | — | no | 22 sequences measured; `include_entries` off by default |
| `search_plant_knowledge` | `query` | no | Optional component; see the caveat above |

`calculate_mixing_protocol` computes and persists nothing, which is why it sits among the read tools despite its name.

Twenty read tools take no `tenant`: `list_tenants` and `get_mcp_activity` are account-scoped, and the species, cultivar, pest, disease, treatment, substrate, glossary, hardiness, phase-definition, phase-sequence, species-lifecycle and knowledge-base catalogs are shared across gardens. The split is per tool, read from each `inputSchema` rather than inferred from the group a tool sits in: `list_substrates` takes no `tenant` while `list_fertilizers` in the same group does, and `get_species_phase_sequence` takes none while `get_plant_phase_status` in the same group does. For the remaining 24 the argument follows the tenancy rule above — optional on a key with one membership, required on a key with several — so a recipe cannot infer from this section that omitting it is ever safe.
...
| `transition_plant_phase` | `plant_key`, `target_phase_key` | Move a plant into a phase of its resolved sequence, or repair a wrong one |
| `assign_species_phase_sequence` | `species_key`, `sequence_key` | Bind a species to an existing sequence — **takes no `tenant`**, see below |
| `assign_nutrient_plan` | `plant_key`, `plan_key` | Bind an existing nutrient plan to a plant |
| `record_feeding_event` | `plant_key`, `volume_applied_liters` | Record a fertigation: amount, EC/pH before and after, runoff |
| `create_inspection` | `plant_key` | Record an IPM inspection: pressure level, symptoms, structured findings |
...
Measured: every state-changing tool exposes `dry_run` and `idempotency_key`, and no read tool carries either — which is the cheapest way to tell the two classes apart in a `tools/list` response. This held across all twelve on 2026-08-16, so it is the one classifier that has survived every catalog growth.

`tenant` does **not** separate them, and as of 2026-08-16 it no longer even holds within the write class. Eleven of the twelve state-changing tools take `tenant`, and so do 24 of the 44 reads. The exception is **`assign_species_phase_sequence`, which takes none**: both of its arguments name rows in catalogs shared across gardens — a species and a phase sequence — so the call is not scoped to a garden at all. Its effect is not local either: rebinding a species changes the resolved sequence for that species in *every* tenant. A recipe that treats "no `tenant` argument" as "harmless" has the reasoning backwards here.

For the other eleven, writes are tenant-scoped like everything else and the tenancy rule above applies in full: on a key covering several gardens, omitting `tenant` on a write is an error, not a default.
...
- **MUST** name every forbidden state-changing tool individually in the recipe's `prompt` when the recipe is read-only — `instructions` alone is not enforced on a headless run. Successive catalog measurements raised that list from four to seven and then to twelve: `confirm_care_task`, `archive_plant`, `set_plant_location`, `create_site`, `add_plant_diary_entry`, `claim_diary_analysis`, `submit_diary_analysis`, `transition_plant_phase`, `assign_species_phase_sequence`, `assign_nutrient_plan`, `record_feeding_event`, `create_inspection` (eleven `mcp.write` plus the one `mcp.setup`). Naming a subset is the failure this rule exists to catch, and a category ("and anything else that writes") does not substitute for any of the twelve: the prohibition must not depend on the model's classification. The list has grown twice; a recipe written against an earlier count is a subset today, which is why the machine check below carries the rule rather than this paragraph. A write-capable `-apply` recipe is the exception the word *forbidden* already carries — the tools it exists to call are declared, not prohibited, so it splits the twelve across `Permitted by name:` and `Forbidden by name:` and completeness is a property of that union rather than of the forbidden half. The same MUST binds a project skill that states a tool policy of its own: a recipe's `prompt` does not reach into the skill it loads, so a skill naming seven of twelve is the same subset defect one layer down. A skill that names none of the twelve is governed by the recipe that loads it and is out of scope; naming any of them is what brings a skill under this rule, not whether it happens to carry a block. In every case the names have to sit **inside** the block — a name mentioned in the surrounding prose is not a prohibition, and the machine check counts only what the block holds
...
- The forbidden-tool criterion has now been missed three times by the same mechanism, and the third time was inside the change that fixed the second. The 2026-08-07 measurement raised the list to seven and the recipes were brought up to it. The 2026-08-16 measurement raised it to twelve, and **nine of the ten shipped recipes were measured naming seven** — every one written against the older count had silently become a subset. The first correction pass in that same change fixed only three, because it took the recipe list from an earlier revision of *this paragraph* instead of measuring the recipes; a review caught the remaining six. All ten now name twelve.
...
- This spec has no mechanism for detecting that the live server offers tools it does not list. As of the 2026-08-16 re-inventory the two agree — all 56 are listed here — but nothing keeps them agreeing. The drift has now been caught by hand twice: three days after it opened in August's first week, and nine days after it opened the second time, on both occasions incidentally rather than by a check. Separately, and unrelated to that gap: upstream still documents the catalog as growing toward roughly 30, which the implementation has passed twice over.
...
- Per-tool `tenant`, `dry_run` and `idempotency_key` classification derived by testing each tool's `inputSchema.properties` for those keys — not from the grouping in this document. For the 43 tools of 2026-08-07 this came from the `tools/list` response with nothing invoked, so those claims are about the declared schema rather than observed behaviour; for the 13 added by 2026-08-16 the schemas were read from the surface a Goose session is given, which is that response as the client presents it
...
- Live measurement of the reference instance, 2026-08-16, during a plant-configuration audit rather than a dedicated probe: 56 tools on the session surface, read-only `tools/call` for `list_tenants`, `list_plants`, `get_plant`, `list_plants_at_location`, `get_plant_diagnostics`, `get_plant_phase_status`, `get_plant_phase_history`, `get_species_info`, `get_species_lifecycle`, `get_species_phase_sequence`, `list_phase_sequences`, `list_species`, `list_cultivars`, `list_substrates`, `list_fertilizers`, `list_nutrient_plans`, `get_nutrient_plan`, `get_plant_nutrient_plan`, `calculate_mixing_protocol`, `get_due_care_tasks`, `list_tasks`, `get_harvest_readiness`, `list_planting_runs`, plus `get_mcp_activity` and `search_plant_knowledge`, which both failed as recorded above. **Write tools were called this time**, unlike in the earlier measurements: `assign_species_phase_sequence` and `transition_plant_phase` succeeded, `assign_nutrient_plan` failed with `INTERNAL_ERROR` on the commit path while its `dry_run` succeeded (`nolte/kamerplanter#1145`). So `dry_run` is confirmed as a genuine no-op on at least one write, and confirmed **not** to be evidence that the write itself will succeed
